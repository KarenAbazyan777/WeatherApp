...
import './Show15daysWeather.css'
import Search from '../Search/Search'
import {NavLink, useNavigate} from 'react-router-dom';


const Show15DaysWeather = observer(() => {
    ...
    const navigetion = ()=>{
        if(!currentWeather.error){
            ...
            navigate(`/days-15/${placeStore.weather}`);
        }
    }
                        ...
                        {days.map((piece,index)=>{  
                            return  (
                                    ...
                                    <NavLink to={`/daily/${placeStore.weather}/${piece.datetime}`} className='Link' key={piece.datetime.substring(5,10)}>
                                        <h4 className='qqq'>{piece.datetime.substring(5,10)}</h4>
...
export default Show15DaysWeather;