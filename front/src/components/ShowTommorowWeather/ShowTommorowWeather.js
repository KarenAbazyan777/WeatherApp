...
const ShowTommorowWeather = observer(() => {
    ...
    useEffect(()=>{
        ...
        fetch(` http://api.weatherapi.com/v1/forecast.json?key=1eea7f8c85344cf1862213549232004&q=${placeStore.weather}&days=2`)
        .then(res=>{
            if(!res.ok){
                throw new Error();
            }
    ...
    const navigetion = ()=>{
        if(!currentWeather.error){
            ...
            navigate(`/tommorow/${placeStore.weather}`);
        }
    }
    ...
    if(toJS(currentWeather.currentWeather.length !== 0 && 'forecast' in toJS(currentWeather.currentWeather[toJS(currentWeather.currentWeather.length) - 1]))){ 
        ...
        if(item.forecast.forecastday.length === 2){
            const hour = item.forecast.forecastday[1].hour;
        return(
            <div className='appdaily'>
                ...
                <div className='item'>
                    <div className='placename'>
                        ...
                        <h4>{item.location.country} today {item.forecast.forecastday[1].date}</h4>
                    </div>
...
export default ShowTommorowWeather;